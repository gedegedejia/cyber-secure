...
def urlReport(arg):
    ...
    # 对于 URL 分析报告，virustotal 需要“URL 标识符”或 URL 的 Base64 表示形式进行扫描（无填充）

    # 根据存储在 target_url 中的用户输入创建virustotal“url 标识符”
    # 将用户提交的url编码为base64并去掉末尾的“==”
    url_id = base64.urlsafe_b64encode(target_url.encode()).decode().strip("=")

    #print(url_id)

    # 修改virustotal apiv3 url以包含唯一生成的url_id
    url = "https://www.virustotal.com/api/v3/urls/" + url_id

    print(url)

    # 不建议将直接为“x-apikey”输入 API 密钥，应在 .env 文件中单独存储和访问
    headers = {
